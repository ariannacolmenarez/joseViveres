...
class ingresoModel extends Conexion{
    ...
    // public function registrarIngreso(IngresoModel $p){
    //     try {
    //             $consulta="INSERT INTO Ingreso_productos(
    //                 nombre , 
    //                 descripcion,
    ...
    // public function guardar(ingresoModel $p){
    //     try {
    ...
    //         // ));
    //         $id_ingreso=$p->getid();
    //         $consulta1= Conexion::conect()->prepare("SELECT id_producto FROM ingreso_detalles WHERE id_ingreso=$id_ingreso");
    //         $consulta1->execute();
    //         $r= $consulta1->fetchAll(PDO::FETCH_ASSOC);    

    //         if ($consulta1->rowCount() > 0) {
    //             foreach ($p->getproductos()as $prod) {
    //                 var_dump($prod['id']);
    //                 var_dump("------------");
    //                 foreach($r as $row){
    //                     if ($row['id_producto']== $prod['id'] ) {

    //                             var_dump("igual".$prod['id']);
    //                     }else{

    //                         echo "diferente".$prod['id'];
    //                         // var_dump(array_filter($row, function($v, $k) use ($prod) {
    //                         //     var_dump($v);
    //                         //     var_dump("prod".$prod['id']);
    //                         //     if ($v == $prod['id']) {
    //                         //         var_dump("igual ".$prod['id']);
    //                         //     }
    //                         //     // var_dump($k);
    //                         // }, ARRAY_FILTER_USE_BOTH));
    //                         // var_dump(array_diff($prod, $row));

    //                         // if (in_array($prod['id'], $row)) {
    //                         //     echo "igual else ".$prod['id'];
    //                         // }else{
    //                         //     echo "diferente else ".$prod['id'];
    //                         // }
    //                         // foreach($consulta1 as $row2){

    //                         //     if ($row2['id_producto']==$prod['id']) {
    //                         //         var_dump("igual ".$prod['id']);
    //                         //     }else{
    //                         //         var_dump("diferente else ".$prod['id']);
    //                         //     }
    //                         // }
