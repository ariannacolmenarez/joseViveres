<?php
    ...
    {
        $salida = FALSE;
        $password = hash('sha256', _CODIGO_PASSWORD_); //Genera Valor Cifrado en base a un string
        $vectorInicializacion = substr(hash('sha256', _CODIGO_VECTOR_), 0, 16);
        $salida = openssl_encrypt($cadena, _METODO_, $password, 0, $vectorInicializacion);
    ...
    {
        $password = hash('sha256', _CODIGO_PASSWORD_);
        $vectorInicializacion = substr(hash('sha256', _CODIGO_VECTOR_), 0, 16);
        $salida = openssl_decrypt(base64_decode($cadena), _METODO_, $password, 0, $vectorInicializacion);
        return $salida;
    }
